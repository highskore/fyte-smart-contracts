...
import "src/dependencies/interfaces/IDiamondCut.sol";
import "src/dependencies/facets/DiamondCutFacet.sol";
import "src/dependencies/facets/DiamondLoupeFacet.sol";
import "src/dependencies/facets/OwnershipFacet.sol";
import "src/dependencies/Diamond.sol";
...
abstract contract StateCacheBug is StateDeployDiamond {
    ...
    function setUp() public virtual override {
        ...
        for (uint256 i = 0; i < selectorsAdd.length; i++) {
            selectorsAdd[i] = selectors[i];
        }

        cut[0] =
            FacetCut({ facetAddress: address(test1Facet), action: FacetCutAction.Add, functionSelectors: selectorsAdd });
        ...
        cut[0] =
            FacetCut({ facetAddress: address(0x0), action: FacetCutAction.Remove, functionSelectors: newSelectors });
